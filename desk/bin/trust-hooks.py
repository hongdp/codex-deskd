    ...
    for gi, grp in enumerate(groups):
        for hi, h in enumerate(grp.get("hooks", [])):
            ...
            if h.get("statusMessage"):
                norm["statusMessage"] = h["statusMessage"]
            if h.get("additionalContextLimit") not in (None, 2500):
                norm["additionalContextLimit"] = h["additionalContextLimit"]
            identity = {"event_name": label(event), "hooks": [norm]}
            if grp.get("matcher"):
                identity["matcher"] = grp["matcher"]
            ser = json.dumps(
                canon(identity), separators=(",", ":"), ensure_ascii=False
            ).encode()
