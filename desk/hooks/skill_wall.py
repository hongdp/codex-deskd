...
CONTRACT = re.compile(
    r"(^|[^\w.-])desk/(AGENTS\.md|config\.toml|agents/|rules/|hooks/)"
)
ORDER_TOOLS = {
    "place_equity_order",
    "place_option_order",
...
WRITE_HINTS = re.compile(
    r"(>|\btee\b|\bmv\b|\bcp\b|\brm\b|\btouch\b|\bchmod\b|\bchown\b|"
...
if CONTRACT.search(blob):
    ...
    if t in {"shell", "bash", "exec_command"} and not read_only(command_text(inp)):
        deny(
            f"{role} may not edit the desk contract (read-only access is allowed); propose a diff instead"
        )
